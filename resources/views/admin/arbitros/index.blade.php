...
@section('content')
    <div class="card card-outline card-danger">
        <div class="card-header">
            <div class="row justify-content-end">
                <div class="btn-group">
                    <a href="{{ route('admin.arbitros.create') }}" class="btn btn-dark btn-sm"><span class="fa fa-plus"></span><br>{{__('Nuevo')}}</a>
                </div>
            </div>
        </div>
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-striped" id="datatable-arbitros" style="width:100%">
                    <thead>
                        <th class="text-center" width="50px">#</th>
                        <th class="text-center">{{__('Nombre')}}</th>
                        <th class="text-center">{{__('Edad')}}</th>
                        <th class="text-center">{{__('Nacionalidad')}}</th>
    ...
    @include('admin.arbitros.modales._delete')
@stop

@section('js')
<script>
    function montarTabla() {
        return $('#datatable-arbitros').DataTable({
            "order": [[0, "asc"]],
            "stateSave": true,
            "serverSide": true,
            "processing": true,
            ...
            "ajax": {
                url: "{{ route('admin.arbitros.indexAjax') }}",
                method: "get",
            },
            "columns": [
                ...
                {data: null,'className': 'text-center',
                    render: function(data,type,row,meta){
                        ...
                        url = '{{ url('admin/arbitros/edit') }}/'+data.id;
                        button += '<a href="'+url+'" class="btn btn-dark" title="{{__('Editar')}}"><i class="fa fa-edit"></i></a> '
                        button += '<button type="button" id="'+data.id+'" class="btn btn-danger eliminar-arbitro" title="{{__('Eliminar')}}" data-toggle="modal" data-target="#modalEliminar"><i class="fa fa-trash"></i></button>'
    ...
    $(document).ready(function(){
        ...
        $('#datatable-arbitros tbody').on('click', '.eliminar-arbitro', function () {
            let id = $(this).attr('id');
            let url = "{{ url('/admin/arbitros/destroy') }}/" + id;
            $('#formularioEliminarArbitro').attr('action', url);
        });
    });
</script>
@stop
