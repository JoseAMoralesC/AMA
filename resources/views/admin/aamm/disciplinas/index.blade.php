...
@section('content')
    <div class="col-md-12 card card-outline card-danger">
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-striped" id="datatable-disciplinas" style="width:100%">
                    <thead>
    ...
    @include('admin.aamm.disciplinas.modales._delete')
@stop

@section('js')
<script>
    function montarTabla() {
        return $('#datatable-disciplinas').DataTable({
            "order": [[0, "asc"]],
            "stateSave": true,
            "serverSide": true,
            "processing": true,
            ...
            "ajax": {
                url: "{{ route('admin.disciplinas.indexAjax') }}",
                method: "get",
            },
            "columns": [
                ...
                {data: null,'className': 'text-center',
                    render: function(data,type,row,meta){
                        ...
                        url = '{{ url('admin/disciplinas/edit') }}/'+data.id;
                        button += '<a href="'+url+'" class="btn btn-dark" title="{{__('Editar')}}"><i class="fa fa-edit"></i></a> '
                        button += '<button type="button" id="'+data.id+'" class="btn btn-danger eliminar-disciplina" title="{{__('Eliminar')}}" data-toggle="modal" data-target="#modalEliminar"><i class="fa fa-trash"></i></button>'
    ...
    $(document).ready(function(){
        ...
        $('#datatable-disciplinas tbody').on('click', '.eliminar-disciplina', function () {
            let id = $(this).attr('id');
            let url = "{{ url('/admin/disciplinas/destroy') }}/" + id;
            $('#formularioEliminarDisciplina').attr('action', url);
        });
    });
</script>
@stop
