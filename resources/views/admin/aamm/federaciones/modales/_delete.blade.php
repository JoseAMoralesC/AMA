    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="staticBackdropLabel">{{ __('Eliminar federacion') }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                {{ __('¿Estas seguro de que quieres eliminar esta federacion?') }}
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-dark" data-dismiss="modal">{{ __('No') }}</button>
                {{ Form::open(array('method' => 'post', 'id' => 'formularioEliminarFederacion')) }}
                    {{ method_field('DELETE') }}
                    {{ Form::token() }}
