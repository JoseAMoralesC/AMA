...
@section('content')
    <div class="col-md-12 card card-outline card-danger">
        <div class="card-header">
            <div class="row justify-content-end">
                <div class="btn-group">
                    <a href="{{ route('admin.federaciones.create') }}" class="btn btn-dark btn-sm"><span class="fa fa-plus"></span><br>{{__('Nueva')}}</a>
                </div>
            </div>
        </div>
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-striped" id="datatable-federaciones" style="width:100%">
                    <thead>
    ...
    @include('admin.aamm.federaciones.modales._delete')
@stop

@section('js')
<script>
    function montarTabla() {
        return $('#datatable-federaciones').DataTable({
            "order": [[0, "asc"]],
            "stateSave": true,
            "serverSide": true,
            "processing": true,
            ...
            "ajax": {
                url: "{{ route('admin.federaciones.indexAjax') }}",
                method: "get",
            },
            "columns": [
                ...
                {data: null,'className': 'text-center',
                    render: function(data,type,row,meta){
                        ...
                        url = '{{ url('admin/federaciones/edit') }}/'+data.id;
                        button += '<a href="'+url+'" class="btn btn-dark" title="{{__('Editar')}}"><i class="fa fa-edit"></i></a> '
                        button += '<button type="button" id="'+data.id+'" class="btn btn-danger eliminar-federacion" title="{{__('Eliminar')}}" data-toggle="modal" data-target="#modalEliminar"><i class="fa fa-trash"></i></button>'
    ...
    $(document).ready(function(){
        ...
        $('#datatable-federaciones tbody').on('click', '.eliminar-federacion', function () {
            let id = $(this).attr('id');
            let url = "{{ url('/admin/federaciones/destroy') }}/" + id;
            $('#formularioEliminarFederacion').attr('action', url);
        });
    });
</script>
@stop
