    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="staticBackdropLabel">{{ __('Eliminar estilo') }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                {{ __('¿Estas seguro de que quieres eliminar este estilo?') }}
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-dark" data-dismiss="modal">{{ __('No') }}</button>
                {{ Form::open(array('method' => 'post', 'id' => 'formularioEliminarEstilo')) }}
                    {{ method_field('DELETE') }}
                    {{ Form::token() }}
