...
@section('content')
    <div class="card card-outline card-danger">
        <div class="card-header">
            <div class="row justify-content-end">
                <div class="btn-group">
                    <a href="{{ route('admin.cuotas.create') }}" class="btn btn-dark btn-sm"><span class="fa fa-plus"></span><br>{{__('Nueva')}}</a>
                </div>
            </div>
        </div>
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-striped" id="datatable-cuotas" style="width:100%">
                    <thead>
                        <th class="text-center" width="50px">#</th>
                        <th class="text-center">{{__('Nombre')}}</th>
                        <th class="text-center">{{__('Precio')}}</th>
                        <th class="text-center">{{__('Meses de suscripcion')}}</th>
    ...
    @include('admin.cuotas.modales._delete')
@stop

@section('js')
<script>
    function montarTabla() {
        return $('#datatable-cuotas').DataTable({
            "order": [[0, "asc"]],
            "stateSave": true,
            "serverSide": true,
            "processing": true,
            ...
            "ajax": {
                url: "{{ route('admin.cuotas.indexAjax') }}",
                method: "get",
            },
            "columns": [
                ...
                {data: 'meses', 'className': 'text-center'},
                {data: null,'className': 'text-center',
                    render: function(data,type,row,meta){
                        let url;
                        let button = "";

                        url = '{{ url('admin/cuotas/edit') }}/'+data.id;
                        button += '<a href="'+url+'" class="btn btn-dark" title="{{__('Editar')}}"><i class="fa fa-edit"></i></a> '
                        button += '<button type="button" id="'+data.id+'" class="btn btn-danger eliminar-cuota" title="{{__('Eliminar')}}" data-toggle="modal" data-target="#modalEliminar"><i class="fa fa-trash"></i></button>'
    ...
    $(document).ready(function(){
        ...
        $('#datatable-cuotas tbody').on('click', '.eliminar-cuota', function () {
            let id = $(this).attr('id');
            let url = "{{ url('/admin/cuotas/destroy') }}/" + id;
            $('#formularioEliminarCuota').attr('action', url);
        });
    });
</script>
@stop
