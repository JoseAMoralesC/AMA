...
class StoreController extends Controller
{
    ...
    public function store(Request $request){
        ...
        try{
            ...
            $this->cursoService->store($dataRequest);
            DB::connection()->commit();
        }catch(\Exception $e){
            DB::connection()->rollBack();
            Log::error($e);

            return redirect()->route('admin.cursos.index')->with('error','El curso '.$request->nombre.' no se ha podido añadir');
        }

        return redirect()->route('admin.cursos.index')->with('success','El curso '.$request->nombre.' se ha añadido con exito');
    }
}
