...
class StoreController extends Controller
{
    ...
    public function store(Request $request){
        $request->validate([
            ...
            'direccion' => 'required',
            'cod_postal' => 'required',
            'email' => 'required|unique:gimnasios,email'
        ],
        [
        ...
        try{
            ...
            $this->gimnasioService->store($dataRequest);
            DB::connection()->commit();
        }catch(\Exception $e){
            DB::connection()->rollBack();
            Log::error($e);

            return redirect()->route('admin.gimnasios.index')->with('error','El gimnasio '.$request->nombre.' no se ha podido añadir');
        }

        return redirect()->route('admin.gimnasios.index')->with('success','El gimnasio '.$request->nombre.' se ha añadido con exito');
    }
}
