...
class StoreController extends Controller
{
    ...
    public function store(Request $request){
        ...
        try{
            ...
            $this->cuotaService->store($dataRequest);
            DB::connection()->commit();
        }catch(\Exception $e){
            DB::connection()->rollBack();
            Log::error($e);

            return redirect()->route('admin.cuotas.index')->with('error','La cuota '.$request->nombre.' no se ha podido añadir');
        }

        return redirect()->route('admin.cuotas.index')->with('success','La cuota '.$request->nombre.' se ha añadido con exito');
    }
}
